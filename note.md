...
// i++ 在计算时，会创建一个临时变量(num)，++ 在 i 后面时，java会先将 i 自增前的数值储存进临时变量(num = i)，然后对 i 进行自增(i = i + 1)，最后用临时变量(num)进行运算。
...
// ++i 在计算时，会创建一个临时变量(num)，++ 在 i 前面时，java会先对 i 进行自增，然后将 i 自增后的数值储存进临时变量(num = 1)，最后用临时变量(num)进行运算。
```
...
1. 顺序结果，即从上往下执行
2. 分支结构，实例: [单击见代码](IfCase.java)
3. 循环结构，实例：[单击见代码](LoopCase.java)

注意: 死循环会引发CPU飙高问题

## 跳转控制语句

跳转控制语句break、continue，实例:[单击见代码](JumpControlStatement.java)

## 随机数

生成随机数，实例:[单击见代码](CreateRandom.java)
